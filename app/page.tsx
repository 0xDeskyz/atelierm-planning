...
function Button({ children, className = "", variant = "default", size = "md", ...props }: any) {
  ...
  const variants: Record<string, string> = { default: "bg-black text-white hover:bg-black/90", outline: "border border-neutral-300 hover:bg-neutral-50", ghost: "hover:bg-neutral-100" };
...
function Card({ children, className = "" }: any) { return <div className={cx("border rounded-xl bg-white", className)}>{children}</div>; }
function CardContent({ children, className = "" }: any) { return <div className={cx("p-4", className)}>{children}</div>; }
function Input(props: any) { return <input {...props} className={cx("w-full h-9 px-3 rounded-md border border-neutral-300 focus:outline-none focus:ring-2 focus:ring-sky-400", props.className)} />; }
function Textarea(props: any) { return <textarea {...props} className={cx("w-full min-h-28 p-3 rounded-md border border-neutral-300 focus:outline-none focus:ring-2 focus:ring-sky-400", props.className)} />; }

// Tabs
const TabsCtx = React.createContext<{ value: string; onValueChange: (v: string) => void } | null>(null);
function Tabs({ value, onValueChange, children }: any) { return <TabsCtx.Provider value={{ value, onValueChange }}>{children}</TabsCtx.Provider>; }
function TabsList({ children, className = "" }: any) { return <div role="tablist" aria-label="Vues" className={cx("inline-flex gap-1 bg-neutral-100 p-1 rounded-lg", className)}>{children}</div>; }
function TabsTrigger({ value, children }: any) {
  const ctx = (React.useContext(TabsCtx) || { value: undefined, onValueChange: () => {} }) as any;
  const active = ctx.value === value;
  return (
    <button role="tab" aria-selected={active} onClick={() => ctx.onValueChange(value)} className={cx("px-3 py-1.5 rounded-md text-sm", active ? "bg-white shadow text-black" : "text-neutral-600 hover:bg-white/70")}>{children}</button>
  );
}

// Dialog (basique)
function Dialog({ open, onOpenChange, children }: any) { if (!open) return null; return (<div className="fixed inset-0 z-50 flex items-center justify-center"><div className="absolute inset-0 bg-black/40" onClick={() => onOpenChange?.(false)} />{children}</div>); }
function DialogContent({ children, className = "" }: any) { return <div className={cx("relative z-10 w-full max-w-lg rounded-xl bg-white p-4 shadow-lg", className)}>{children}</div>; }
function DialogHeader({ children }: any) { return <div className="mb-2">{children}</div>; }
function DialogTitle({ children }: any) { return <div className="text-lg font-semibold">{children}</div>; }
function DialogFooter({ children }: any) { return <div className="mt-3 flex items-center justify-end gap-2">{children}</div>; }

// ==================================
// Constantes & Démo
// ==================================
// Palette étendue (>=17 couleurs)
const COLORS = [
  "bg-rose-500","bg-amber-500","bg-emerald-500","bg-sky-500","bg-violet-500","bg-pink-500","bg-indigo-500",
  "bg-cyan-500","bg-lime-500","bg-teal-500","bg-orange-500","bg-fuchsia-500","bg-blue-600","bg-red-400","bg-yellow-400","bg-green-400","bg-purple-400","bg-slate-500"
];
...
const toLocalKey = (d: Date) => `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`; // évite UTC shift
function startOfISOWeekLocal(d: Date){ const c=new Date(d); const day=(c.getDay()+6)%7; c.setDate(c.getDate()-day); c.setHours(0,0,0,0); return c; }
function getWeekDatesLocal(anchor: Date){ const start=startOfISOWeekLocal(anchor); return Array.from({length:7},(_,i)=>{ const dd=new Date(start); dd.setDate(start.getDate()+i); return dd;}); }
function getISOWeekAndYear(date: Date){ const tmp = new Date(date); tmp.setHours(0,0,0,0); const day = (tmp.getDay()+6)%7; tmp.setDate(tmp.getDate()-day+3); const isoYear = tmp.getFullYear(); const jan4 = new Date(isoYear,0,4); const jan4Day = (jan4.getDay()+6)%7; const week1Start = new Date(jan4); week1Start.setDate(jan4.getDate()-jan4Day); const diffMs = tmp.getTime() - week1Start.getTime(); const week = 1 + Math.floor(diffMs / (7*24*3600*1000)); return { week, isoYear }; }
...
function getMonthWeeks(anchor: Date){ const first = new Date(anchor.getFullYear(), anchor.getMonth(), 1); const gridStart = startOfISOWeekLocal(first); return Array.from({length:6},(_,w)=>{ const start=new Date(gridStart); start.setDate(gridStart.getDate()+w*7); return Array.from({length:7},(__,i)=>{ const dd=new Date(start); dd.setDate(start.getDate()+i); return dd;}); }); }
const cellKey = (siteId: string, dateKey: string) => `${siteId}|${dateKey}`;
const NOTE_BG: Record<string,string> = { yellow: "bg-yellow-200", green: "bg-green-200", blue: "bg-blue-200", pink: "bg-pink-200", orange: "bg-orange-200" };
const NOTE_RING: Record<string,string> = { yellow: "ring-yellow-300", green: "ring-green-300", blue: "ring-blue-300", pink: "ring-pink-300", orange: "ring-orange-300" };
...
// ==================================
// Draggable Person Chip (panel gauche)
// ==================================
...
// ==================================
// Assignment chip (draggable entre cellules, sans duplication)
// ==================================
...
// ==================================
// Droppable Cell (Day x Site) – enrichie
// ==================================
function DayCell({ date, site, assignments, people, onEditNote, notes, onRemoveAssignment }: any) {
  ...
  const highlight = meta.highlight && PASTELS[meta.highlight] ? meta.highlight : null;
  const unavailable = Boolean(meta.holiday || meta.blocked);

  return (
    <div
      className={cx(
        "border min-h-20 p-2 relative rounded-xl bg-white",
        isOver ? "ring-2 ring-sky-400" : "",
        highlight ? `${PASTELS[highlight].bg} ${PASTELS[highlight].ring} ring-2` : "border-neutral-200",
    ...
    >
      ...
      {/* badges dispo */}
      {meta.holiday && (
        <div className="absolute top-1 left-1 text-[10px] px-1.5 py-0.5 rounded bg-red-100 text-red-800 border border-red-200">Férié</div>
      )}
      {meta.blocked && !meta.holiday && (
        <div className="absolute top-1 left-1 text-[10px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-200">Indispo</div>
      )}
      ...
      <div className="flex flex-wrap gap-1 pr-6">
        {todays.map((a: any) => {
          ...
          return p ? (
            <AssignmentChip key={a.id} a={a} person={p} onRemove={() => onRemoveAssignment(a.id)} />
          ) : null;
        })}
      </div>

      {/* icône note si juste du texte principal */}
      {(meta.text && !meta.holiday && !meta.blocked) && (
        <FileText className="w-4 h-4 text-amber-500 absolute top-1 left-1" title={meta.text} />
      )}

      {/* bas-gauche texte */}
      {meta.blText && (
        <div className="absolute bottom-1 left-2 text-[11px] text-neutral-700 max-w-[70%] truncate" title={meta.blText}>{meta.blText}</div>
      )}

      {/* bas-droit mini post-it */}
      {meta.brNote?.text && (
        <div className={cx("absolute bottom-1 right-1 text-[10px] px-1.5 py-0.5 rounded shadow border", meta.brNote.color && PASTELS[meta.brNote.color]?.bg)} title={meta.brNote.text}>
          {meta.brNote.text}
        </div>
      )}

      {/* bouton édition */}
      <button onClick={() => onEditNote(date, site)} className="absolute bottom-1 right-7 opacity-70 hover:opacity-100" aria-label="Éditer la case" title="Éditer la case">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4 text-sm">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={holiday} onChange={(e)=>setHoliday(e.target.checked)} /> Jour férié
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={blocked} onChange={(e)=>setBlocked(e.target.checked)} /> Chantier indisponible
            </label>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              {(["mint","sky","peach"] as const).map((c)=> (
                <button key={c} type="button" onClick={()=>setHighlight(c)} className={cx("px-2 py-1 text-sm rounded border", PASTELS[c].bg, highlight===c ? "ring-2 ring-black" : "")}>
                  {c}
                </button>
              ))}
            </div>
...
export default function Page() {
  ...
  const [notes, setNotes] = useState<Record<string, any>>({}); // meta par case `${siteId}|${dateKey}`
  const [absencesByWeek, setAbsencesByWeek] = useState<Record<string, Record<string, boolean>>>({}); // { "YYYY-W##": { personId: true } }

  // View / navigation (Semaine / Mois uniquement)
  ...
  const toggleAbsentThisWeek = (pid: string) => { setAbsencesByWeek((prev) => { const week = { ...(prev[currentWeekKey] || {}) }; week[pid] = !week[pid]; return { ...prev, [currentWeekKey]: week }; }); };

  const onDragEnd = (e: any) => {
    const { active, over } = e;
    if (!over || !active?.data?.current) return;
    const data = active.data.current; // { type, ... }
    ...
    if (cellMeta.holiday || cellMeta.blocked) return; // case indisponible

    if (data.type === "person" && data.person) {
      const person = data.person;
      if (isAbsentOnWeek(person.id, wkKey)) return; // absent => ignore
      const hasSameDay = assignments.some((a) => a.personId === person.id && a.date === targetDateKey);
      if (hasSameDay) return; // pas 2 affectations le même jour
    ...
    if (data.type === "assignment" && data.assignmentId) {
      ...
      if (!ass) return;
      if (isAbsentOnWeek(ass.personId, wkKey)) return; // bloqué par absence
  ...
  const saveNote = (val: any) => { if (noteKeyState) setNotes((prev) => ({ ...prev, [noteKeyState]: val })); };
  ...
  const removeSite = (id: string) => {
    ...
    setNotes((prev) => { const next = { ...prev }; Object.keys(next).forEach((k) => { if (k.startsWith(`${id}|`)) delete (next as any)[k]; }); return next; });
  };
            ...
            <Card>
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <AddPerson onAdd={(name: string, color: string) => setPeople((p) => [...p, { id: (typeof crypto !== 'undefined' && (crypto as any).randomUUID) ? (crypto as any).randomUUID() : `p${Date.now()}`, name, color }])} />
                </div>
                <div className="space-y-2">
                  {people.map((p) => (
            ...
            <Card>
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <AddSite onAdd={(name: string) => setSites((s) => [...s, { id: (typeof crypto !== 'undefined' && (crypto as any).randomUUID) ? (crypto as any).randomUUID() : `s${Date.now()}`, name }])} />
                </div>
                <div className="space-y-2">
                  {sites.map((s) => (
